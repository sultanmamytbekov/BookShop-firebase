...
import img from "./img/Img_box.svg";
import "./index.scss";
...
const Admin: React.FC = () => {
  ...
  const { setProduct, IF } = useProduct();
  ...
  function getAdmin() {
    if (
      ...
      let admin = JSON.parse(localStorage.getItem("data") as string) || [];
      admin.unshift(obj);
      localStorage.setItem("data", JSON.stringify(admin));
      setProduct(admin);
      setValue({ ...value, url: null });
      setValue({ ...value, productName: "" });
